...
function printstep1form( $dbname = 'wordpress', $uname = 'username', $pwd = 'password', $dbhost = 'localhost', $dbtype = 'mysql', $vhost = 'yes', $prefix = 'wp_' ) {
		...
		<table class="form-table"> 
			...
			<tr>
				...
				<td>
					...
					if (is_dir(dirname(__FILE__) . '/wp-includes/wp-db/')) {
						$dir = dirname(__FILE__) . '/wp-includes/wp-db/';
						$handle = dir($dir);
						while ($filename = $handle->read()) {
							$restricted = array('.','..','.svn');
							$path = realpath($dir.$filename);
							if (is_dir($path) && !in_array($filename,$restricted)) {
								if ($path !== '/') {
									$path .= '/';
								}
								if (is_readable($path)) {
					...
					if (empty($folders)) {
						$folders[] = $dbtype; // should always have at least mysql
					}
					foreach($folders as $folder) {
						// mysql as default
						if ($folder == 'mysql') {
							echo "<option value=\"$folder\" selected=\"true\">MySQL</option>\n";
						} elseif ($folder == 'sqlsrv') {
							echo "<option value=\"$folder\">SQL Server using MS PHP driver (SQLSRV; PHP5 &amp; Win req.)</option>\n";
						} elseif ($folder == 'mssql') {
							echo "<option value=\"$folder\">SQL Server using PHP native driver (MSSQL)</option>\n";
						} else {
...
function step2() {
	...
	$dbtype  = stripslashes($_POST['dbtype']);
	...
	foreach ($wp_config_file as $line) {
		switch ( trim( substr($line,0,16) ) ) {
			...
			case "define('DB_HOST'":
				...
				break;
			case "define('DB_TYPE'":
				fwrite($handle, str_replace("mysql", $dbtype, $line));
			break;
			case "define('VHOST',":
