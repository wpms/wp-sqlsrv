 ...
 * 
 * Modifications for MSSQL by A.Garcia and A.Gentile
 * 
 * @package WordPress
 * @subpackage Database
 * @since 0.71
	...
	function query($query, $translate = true) {
		...
		if ($this->preceeding_query !== false) {
			if (is_array($this->preceeding_query)) {
				foreach ($this->preceeding_query as $p_query) {
					@mssql_query($sub_query);
				}
			} else {
				@mssql_query($this->preceeding_query);
			}
		...
		if (is_array($query)) {
			foreach ($query as $sub_query) {
				$this->_pre_query();
				$this->result = @mssql_query($sub_query);
				$return_val = $this->_post_query($sub_query);
			}
		} else {
			$this->_pre_query();
			$this->result = @mssql_query(($query);
			$return_val = $this->_post_query($query);
		}
		...
		if ($this->following_query !== false) {
			if (is_array($this->following_query)) {
				...
				@mssql_query($this->following_query);
			}
