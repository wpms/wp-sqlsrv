 ...
 *
 * Modifications for SQLSRV (Microsoft PHP SQL Server driver) by A.Garcia and A.Gentile
 *
 * @package WordPress
 * @subpackage Database
 * @since 0.71
...
class wpdb extends SQL_Translations {
	...
	*/
	...
	/**
	 * Perform a SQLSRV database query, using current database connection.
	 *
	...
	function query($query, $translate = true) {
		...
		if ($this->preceeding_query !== false) {
			if (is_array($this->preceeding_query)) {
				foreach ($this->preceeding_query as $p_query) {
					@sqlsrv_query($this->dbh, $p_query);
				}
			} else {
				@sqlsrv_query($this->dbh, $this->preceeding_query);
			}
		...
		if (is_array($query)) {
			foreach ($query as $sub_query) {
				$this->_pre_query();
				$this->result = @sqlsrv_query($this->dbh, $sub_query);
				$return_val = $this->_post_query($this->dbh, $sub_query);
			}
		} else {
			$this->_pre_query();
			$this->result = @sqlsrv_query($this->dbh, $query);
			$return_val = $this->_post_query($this->dbh, $query);
		}
		
		if ($this->following_query !== false) {
			if (is_array($this->following_query)) {
				foreach ($this->following_query as $f_query) {
					@sqlsrv_query($this->dbh, $f_query);
				}
			} else {
				@sqlsrv_query($this->dbh, $this->following_query);
			}
	...
	{
		$tables = $this->get_col("Select Table_name as \"Table name\"
	From Information_schema.Tables
	Where Table_type = 'BASE TABLE' and Objectproperty
	(Object_id(Table_name), 'IsMsShipped') = 0");
		foreach ($tables as $table) {
			echo "Table: ".$table."<br/>";
			$rows = $this->get_col("SELECT COUNT (*) FROM {$table}");
			foreach ($rows as $row) {
				echo "Rows: ".$row."<br/>";
			}
		}
