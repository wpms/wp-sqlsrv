	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				...
				<?php
				$folders = array();
				if (is_dir(ABSPATH . 'wp-includes/wp-db/')) {
					$dir = ABSPATH . 'wp-includes/wp-db/';
					$handle = dir($dir);
				...
				if (empty($folders)) {
					$folders[] = 'mysql'; // should always have at least mysql
				}
	...
	foreach ($configFile as $line_num => $line) {
		switch (substr($line,0,16)) {
			...
			case "define('DB_HOST'":
				...
				break;
			case "define('DB_TYPE'":
				$configFile[$line_num] = str_replace("mysql", $dbtype, $line);
				break;
			case '$table_prefix  =':
